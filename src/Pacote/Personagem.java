...
public class Personagem {
    ...
    public double danoGerado(int tipo_ataque) {

        double d = 0.0;
        double p = 0.17 * (100 - atk / 100);
        double fc = (hp + 0.1 * cHp) / Math.min(500, cHp);
        double fm = (mp + 0.1 * cMp) / Math.min(500, cMp);

        switch (tipo_ataque) {

            case 1:     // Ataque físico simples
                d = Math.max(0, ((atk / 100) * 210 - Math.abs(p * 210)) * fc);
                break;

            case 2:     // Ataque fíico cruzado
                d = Math.max(0, ((atk / 100) * 362 - Math.abs(p * 362)) * fc);
                break;

            case 3:     // Ataque mágico fogo
                d = Math.max(0, ((atk / 100) * 200 - Math.abs(p * 200)) * fm);
                break;

            case 4:     // Ataque mágico gelo
                d = Math.max(0, ((atk / 100) * 350 - Math.abs(p * 350)) * fm);
                break;
        }
        return danoInfligido(d, tipo_ataque);
    }

    public double danoInfligido(double d, int tipo_ataque) {

        switch (tipo_ataque) {

            case 1:     // Ataque fÃ­sico simples
                return d * (100 / (100 + rFisica));

            case 2:     // Ataque fÃ­sico cruzado
                return d * (100 / (100 + rFisica));

            case 3:     // Ataque mÃ¡gico fogo
                return d * (100 / (100 + rFogo));

            case 4:     // Ataque mÃ¡gico gelo
                return d * (100 / (100 + rGelo));
        }
        return 0;
    }
