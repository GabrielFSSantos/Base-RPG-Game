...
public class Mapa {
    ...
    public void move() {
        ...
        for (i = 0; i < reg.yourLocation().getRouts().size(); i++) {
            ...
            System.out.println("(" + (i + 1) + ") " + reg.yourRouts(i).getNome());
        }
    ...
    public void batalha() {
        ...
        System.out.println("\n>>>>>> BATALHAR: <<<<<<");
        for (i = 0; i < reg.yourLocation().getMonstros().size(); i++) {

            System.out.println("(" + (i + 1) + ") " + reg.yourMonster(i).getNome());
        }

        System.out.println("(0) VOLTAR");
        System.out.print("\nEscolha o monstro para batalha: ");
        op = scan.nextInt();

        if (op > 0 && op <= reg.yourLocation().getMonstros().size()) {

        } else {

            System.out.println(">>>> Este monstro não existe!!! <<<<");
